...
- 系統表引用（references）已由 Builder 事先建立完成
- Staging VM 上已有 EC Platform 的 Docker 部署環境
...
若偏好一鍵完成，直接使用 [switch-tenant.sh](../scripts/switch-tenant.sh)：
...
> ⚠️ `.env.production` 的 `VITE_*` 變數會在 `npm run build` 時靜態替換進 JS bundle。**更換後必須重新 build**。
